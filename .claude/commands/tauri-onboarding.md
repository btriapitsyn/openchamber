...
- **Claude (developer)** – drives all technical decisions, delivers complete implementations, maintains documentation, and keeps code comments/tests in English only.
...
- **Decision Making**: Claude chooses the architecture and implementation details — NO TODOs left for “future work” without explicit agreement.
...
- **Claude**: Implement features, update documentation, prepare instructions for Bohdan’s verification pipeline.
...
- **No local servers**: Claude must not attempt to run long-lived processes (`npm run start`, `npm run dev`, etc.).
