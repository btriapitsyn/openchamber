  ...
  const renderFileItem = (file: FileInfo, level: number) => {
    return (
      <div
        key={file.path}
        className={cn(
          "flex items-center gap-2 px-2 py-1.5 rounded hover:bg-accent cursor-pointer text-sm",
  ...
  const renderFileTree = (file: FileInfo, level: number): React.ReactNode => {
    ...
    return (
      <div key={file.path}>
        <div
          className={cn(
            "flex items-center gap-1 px-2 py-1.5 rounded hover:bg-accent cursor-pointer text-sm",
            file.type === 'file' && selectedFiles.has(file.path) && "bg-primary/10"
          )}
          style={{ paddingLeft: `${level * 12 + 8}px` }}
          onClick={(e) => {
    ...
    <DropdownMenu open={open} onOpenChange={setOpen}>
      ...
      >
        <div className="px-3 py-2 border-b">
          <div className="font-medium text-sm">Select Project Files</div>
        </div>
