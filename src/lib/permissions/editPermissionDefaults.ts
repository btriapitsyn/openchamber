...
export const isBashPermissionMap = (value: unknown): value is Record<string, BashPermissionValue | undefined> => {
    return isRecord(value);
};

export const hasBashAskEntry = (permission?: BashPermissionSetting): boolean => {
    if (!permission) {
        return false;
    }
...
export const hasBashDenyEntry = (permission?: BashPermissionSetting): boolean => {
    if (!permission) {
        return false;
    }
    if (typeof permission === 'string') {
