...
- ✅ Terminal sessions now mirror the web Node implementation. `packages/desktop/src-tauri/src/commands/terminal.rs` spawns the user’s login shell with a full terminal environment, clamps the working directory, and emits `{ type: 'data' }` and `{ type: 'exit' }` events through `window.emit`. Writers/child processes are tracked per session so close operations terminate the PTY cleanly, and the renderer listener in `packages/desktop/src/api/terminal.ts` disconnects as soon as the exit payload is delivered. ctrl/option arrow sequences and backspace behave correctly again because the PTY inherits `TERM=xterm-256color`/`COLORTERM=truecolor` plus the login shell’s keymap.
