  ...
  return (
    <div className={cn('flex h-full flex-col', bgClass)}>
      <div className="border-b px-3 pt-4 pb-3">
        <h2 className="text-base font-semibold text-foreground mb-3">{t('providersSidebar.title')}</h2>
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <Button size="sm"
            ...
            aria-label={t('providersSidebar.connectProvider')}
            title={t('providersSidebar.connectProvider')}
          >
      ...
      <ScrollableOverlay outerClassName="flex-1 min-h-0" className="space-y-1 px-3 py-2 overflow-x-hidden">
        {providers.length === 0 ? (
          <div className="py-12 px-4 text-center text-muted-foreground">
            ...
            <p className="typography-ui-label font-medium">{t('providersSidebar.emptyTitle')}</p>
            <p className="typography-meta mt-1 opacity-75">{t('providersSidebar.emptyDescription')}</p>
          </div>
        ) : (
          <>
            {userProviders.length > 0 && (
              <>
                <div className="px-2 pb-1.5 pt-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                  {t('providersSidebar.userProviders')}
                </div>
            ...
            {projectProviders.length > 0 && (
              <>
                <div className={cn('px-2 pb-1.5 text-xs font-semibold uppercase tracking-wide text-muted-foreground', userProviders.length > 0 ? 'pt-3' : 'pt-2')}>
                  {t('providersSidebar.projectProviders')}
                </div>
