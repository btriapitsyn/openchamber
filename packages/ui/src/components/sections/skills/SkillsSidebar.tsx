...
export const SkillsSidebar: React.FC<SkillsSidebarProps> = ({ onItemSelect }) => {
  ...
  const handleConfirmDeleteSkill = async () => {
    ...
    if (success) {
      toast.success(t('skillsSidebar.skillDeletedSuccessfully', { name: deleteDialogSkill.name }));
      setDeleteDialogSkill(null);
    } else {
      toast.error(t('skillsSidebar.failedToDeleteSkill'));
    }
  ...
  const handleDuplicateSkill = async (skill: DiscoveredSkill) => {
    ...
    if (!detail) {
      toast.error(t('skillsSidebar.failedToLoadSkillDetailsForDuplication'));
      return;
    }
  ...
  const handleRenameSkill = async () => {
    ...
    if (!sanitizedName) {
      toast.error(t('skillsSidebar.skillNameRequired'));
      return;
    }
    ...
    if (skills.some((s) => s.name === sanitizedName)) {
      toast.error(t('skillsSidebar.skillAlreadyExists'));
      return;
    }
    ...
    if (!detail) {
      toast.error(t('skillsSidebar.failedToLoadSkillDetails'));
      setRenameDialogSkill(null);
      return;
    }
    ...
    const success = await createSkill({
      name: sanitizedName,
      description: t('skillsSidebar.renamedSkillDescription'),
    ...
    if (success) {
      ...
      if (deleteSuccess) {
        toast.success(t('skillsSidebar.skillRenamedTo', { name: sanitizedName }));
        setSelectedSkill(sanitizedName);
      } else {
        toast.error(t('skillsSidebar.failedToRemoveOldSkillAfterRename'));
      }
    } else {
      toast.error(t('skillsSidebar.failedToRenameSkill'));
    }
    ...
    <div className={cn('flex h-full flex-col', bgClass)}>
      ...
      <ScrollableOverlay outerClassName="flex-1 min-h-0" className="space-y-1 px-3 py-2 overflow-x-hidden">
        {skills.length === 0 ? (
          <div className="py-12 px-4 text-center text-muted-foreground">
            ...
            <p className="typography-ui-label font-medium">{t('skillsSidebar.noSkillsConfigured')}</p>
            <p className="typography-meta mt-1 opacity-75">{t('skillsSidebar.usePlusToCreate')}</p>
          </div>
        ) : (
          <>
      ...
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{t('skillsSidebar.deleteSkillTitle')}</DialogTitle>
            <DialogDescription>
              {t('skillsSidebar.deleteSkillDescription', { name: deleteDialogSkill?.name ?? '' })}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
      ...
      <Dialog open={renameDialogSkill !== null} onOpenChange={(open) => !open && setRenameDialogSkill(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t('skillsSidebar.renameSkillTitle')}</DialogTitle>
            <DialogDescription>
              {t('skillsSidebar.renameSkillDescription', { name: renameDialogSkill?.name ?? '' })}
            </DialogDescription>
          </DialogHeader>
          <Input
            value={renameNewName}
            onChange={(e) => setRenameNewName(e.target.value)}
            placeholder={t('skillsSidebar.newSkillNamePlaceholder')}
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="typography-micro text-muted-foreground bg-muted px-1 rounded flex-shrink-0 leading-none pb-px border border-border/50">
              {skill.scope}
                ...
                {skill.source === 'claude' && (
              <span className="typography-micro text-muted-foreground bg-muted px-1 rounded flex-shrink-0 leading-none pb-px border border-border/50">
                {t('skillsSidebar.sourceClaude')}
              </span>
            )}
                {skill.source === 'agents' && (
              <span className="typography-micro text-muted-foreground bg-muted px-1 rounded flex-shrink-0 leading-none pb-px border border-border/50">
                {t('skillsSidebar.sourceAgents')}
              </span>
            )}
          </div>
        </button>
