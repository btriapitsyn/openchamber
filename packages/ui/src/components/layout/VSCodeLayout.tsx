...
const VSCodeHeader: React.FC<VSCodeHeaderProps> = ({ title, showBack, onBack }) => {
  const { getCurrentModel } = useConfigStore();
  const getContextUsage = useSessionStore((state) => state.getContextUsage);

  const currentModel = getCurrentModel();
  const limits = currentModel?.limit && typeof currentModel.limit === 'object' ? currentModel.limit : {};
