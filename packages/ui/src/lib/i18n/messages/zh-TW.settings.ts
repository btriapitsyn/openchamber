...
'settings.remoteInstances.clientAuth.actions.addDevice': '新增裝置',
  ...
  'settings.openchamber.git.agentAuthority': '在代理 Shell 中套用身分',
  'settings.openchamber.git.agentAuthorityAria': '在代理 Shell 中套用身分',
  'settings.openchamber.git.agentAuthorityInfo': '代理自己執行的 Git 會取得儲存庫的身分而非本機設定，並被要求使用 OpenChamber 工具而非直接傳輸。使用系統 Git 的儲存庫保留本機設定。於下次 OpenCode 重新啟動後生效，絕不影響你自己的終端機。',
  ...
  'settings.sourceControl.agentAuthority.title': '代理 Shell',
  'settings.sourceControl.agentAuthority.label': '在代理 Shell 中套用此儲存庫的身分',
  'settings.sourceControl.agentAuthority.info': '關閉後，代理在此執行的 Git 保留本機設定，Shell 中的直接傳輸也不會被拒絕。於下次 OpenCode 重新啟動後生效。',
  'settings.sourceControl.agentAuthority.machineOff': '本機未在代理 Shell 中套用身分。請在設定 → Git 中開啟。',
