...
During directory activation, the saved OpenChamber defaults for model, agent, and variant remain authoritative while providers are loading or temporarily unavailable. `loadAgents` captures the runtime identity and defaults revision, waits for any provider load, then reconciles the settings again at the final commit boundary; a newer local preference wins over an older read. `loadDesktopSettings` overlays pending and in-flight writes for the captured runtime on both cached and fresh reads. Runtime keys and generations scope the caches, queues, and loader completions, so work from an obsolete runtime cannot publish defaults into the active one. The defaults UI only clears a saved variant when the user explicitly changes the model or variant; a discovery gap is not a preference change.
