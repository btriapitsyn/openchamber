...
`contributes.panel` descreve a entrada na barra lateral. `id` é em kebab-case e precisa ser único entre as extensões instaladas. `icon` é um nome do Remixicon (`RiWindowLine` vira `window`) ou um arquivo SVG dentro da sua pasta, como `icon.svg`. `entry` é o arquivo HTML dentro da sua pasta.
...
Sua página consegue saber em qual dos dois está por `ctx.surface` (`panel` ou `dialog`), então você pode mostrar uma lista na barra lateral e um seletor na janela.
...
Outras três são adicionadas por você automaticamente: `network` quando você declara uma `integration`, `service` quando declara um `service`, e `filesystem` quando declara padrões `filesystem`.
...
Para o painel ficar com a cara do resto do app, use os componentes de `@openchamber/sdk/ui`: botões, campos, dropdowns, abas, listas e mais, com as cores e fontes do app. Veja [Kit de UI](/sdk/ui/). Você também pode escrever seu próprio HTML e CSS.
