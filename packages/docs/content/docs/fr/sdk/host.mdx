...
`onDirectory`, `onSession`, `onSessionLifecycle`, `onConnection` et `onSettings` rejouent la dernière valeur quand vous vous abonnez tard, puis continuent à se déclencher à chaque changement.
...
`providerId` est l'id de votre panneau ; OpenChamber l'écrase de toute façon avec votre id. `id` est votre propre identifiant pour l'élément. `kind` est `issue` (par défaut) ou `pull`. `text` est un contexte optionnel pour le modèle, 1 à 16000 caractères après trim.
