...
Trois autres sont ajoutées pour vous : `network` quand vous déclarez une `integration`, `service` quand vous déclarez un `service`, et `filesystem` quand vous déclarez des motifs `filesystem`.
