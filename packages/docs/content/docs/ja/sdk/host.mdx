...
`onDirectory`、`onSession`、`onSessionLifecycle`、`onConnection`、`onSettings` は、遅れて購読しても最新の値を再送し、その後は変化のたびに発火し続けます。
...
`providerId` はパネル ID です。いずれにせよ OpenChamber があなたの ID で上書きします。`id` は項目に対するあなた自身の識別子です。`kind` は `issue`(デフォルト)または `pull` です。`text` はモデルに渡す任意のコンテキストで、トリム後 1 から 16000 文字です。
