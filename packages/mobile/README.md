...
- Phones and tablets share one navigation model: a sessions drawer/sidebar on the left, the workspace drawer (Changes / Files / Terminal / Server browser / Notes / MCP) on the right, and no overflow menu. Server browser appears when enabled on the connected server. Tablets differ only in that the sessions list is a resizable persistent sidebar and the header dropdowns are anchored popovers.
...
- Server browser uses the same remote Chrome viewer in hosted mobile and Capacitor. Its selected session and page are remembered per server and directory. Closing the workspace, switching tabs, opening an app-level overlay, changing project or server, or disabling the feature disconnects the viewer and releases user control. Backgrounding the app pauses the connection until it is visible again. Reopening checks the saved session and page against the server. Mobile does not launch local Chrome.
