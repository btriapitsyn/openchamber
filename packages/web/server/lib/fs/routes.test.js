...
describe('fs watch', () => {
  ...
  it('watches relocated managed chat directories using the shared workspace policy', async () => {
    const directory = '/data/chats/session-1';
    const { handler, registrations, writeSseEvent } = registerWatch({ managedChatsRoot: '/data/chats' });
    const req = new EventEmitter();
    req.query = { directory: '/repo', directories: JSON.stringify([directory]) };
    const res = createMockResponse();
    ...
    try {
      ...
      expect(registrations.map((entry) => entry.directory)).toEqual([directory]);
      expect(writeSseEvent).toHaveBeenCalledWith(res, {
        type: 'openchamber:files-watch-ready',
        properties: { directories: [directory] },
      });
    } finally {
      req.emit('close');
    }
