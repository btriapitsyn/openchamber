...
- `resolveGitCommonDirectory(directory)`: Resolve and canonicalize the shared Git metadata directory used by a repository and all of its linked worktrees.
- `watchWorktreeChanges(directory, onChange)`: Watch the repository's common Git metadata for linked-worktree changes. The worktree list route starts these watchers before its first authoritative list, keys them by the canonical common Git directory, keeps a bounded set, and emits coarse control events so clients can refresh without polling. Watch setup failure does not affect worktree listing, and graceful shutdown closes every active watcher.
