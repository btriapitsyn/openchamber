...
async function main() {
  ...
  app.use((req, res, next) => {
    if (req.path.startsWith('/api/themes/custom') || req.path.startsWith('/api/config/agents') || req.path.startsWith('/api/git')) {
      // Parse JSON for WebUI endpoints (themes, agent config, git)
